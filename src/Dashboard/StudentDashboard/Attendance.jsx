...
import { Calendar, Clock, CheckCircle, XCircle, AlertCircle, BookOpen, BarChart3, ChevronLeft, ChevronRight } from 'lucide-react';
...
const Attendance = ({ courses = [] }) => {
    ...
    // --- All your helper functions (getDaysInMonth, getStatusColor, etc.) remain unchanged ---
    ...
    const renderCalendar = () => {
        ...
        weekDays.forEach((day, index) => {
            days.push(
                <div key={day} className="text-center text-xs font-semibold text-gray-500">
                    {/* Show full name on sm screens and up, single letter on extra small */}
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            days.push(
                <div
                    key={day}
                    // Responsive classes for calendar days
                    className={`
                        py-2 px-1 sm:p-2 rounded-lg text-center text-xs sm:text-sm font-medium cursor-pointer transition-colors duration-200 relative
        ...
        <div className="space-y-6">
            ...
            <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6">
                <div className="flex items-center space-x-3 mb-4">
                    ...
                    <h1 className="text-xl sm:text-2xl font-bold text-gray-800">Attendance Record</h1>
                </div>
                
                {/* Course Filter: already responsive with sm:flex-row */}
                <div className="flex flex-col sm:flex-row sm:items-center space-y-4 sm:space-y-0 sm:space-x-4">
                    ...
                    <select 
                        ...
                        className="w-full p-3 border border-gray-300 rounded-lg bg-white shadow-sm focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all duration-200 text-black"
                    >
            ...
            {/* Main grid: Stacks on mobile, goes to 3 columns on lg screens */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                {/* Calendar Section: adjusted padding */}
                <div className="lg:col-span-2 bg-white rounded-2xl shadow-lg p-4 md:p-6">
                    <div className="flex items-center justify-between mb-4">
                        <h2 className="text-lg sm:text-xl font-bold text-gray-800">{formatDate(currentDate)}</h2>
                    ...
                    <div className="mt-6 flex flex-wrap gap-x-4 gap-y-2 text-sm">
                        <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-green-500 rounded-full"></div><span className="text-gray-600">Present</span></div>
                        <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-yellow-500 rounded-full"></div><span className="text-gray-600">Late</span></div>
                        <div className="flex items-center space-x-2"><div className="w-4 h-4 bg-red-500 rounded-full"></div><span className="text-gray-600">Absent</span></div>
                    </div>
                </div>

                {/* Sidebar section */}
                <div className="space-y-6">
                    {/* Check-in/Check-out Buttons: adjusted padding */}
                    <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6">
                    ...
                    {/* Attendance Summary: adjusted padding */}
                    <div className="bg-white rounded-2xl shadow-lg p-4 md:p-6">
                        <div className="flex items-center space-x-2 mb-4">
