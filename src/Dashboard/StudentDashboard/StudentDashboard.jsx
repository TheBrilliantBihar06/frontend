...
import Navbar from "../../components/Navbar";
import Footer from "../../components/Footer";
import Sidebar from "./Sidebar"; // Your new, enhanced sidebar
import Profile from "./Profile1";
import Attendance from "./Attendance";
import Courses from "./Courses";
import Notes from "./Notes";
...
export default function StudentDashboard() {
  const [activeView, setActiveView] = useState("profile");
  // State to manage sidebar visibility (now used by the Sidebar itself)
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const ActiveComponent = components[activeView];
  
  // A placeholder function for logout logic
  const handleLogout = () => {
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 font-sans">
      ...
      {/* Main layout wrapper */}
      <div className="flex flex-1">
        {/* The new Sidebar component is now self-contained */}
        <Sidebar 
          activeView={activeView}
          setActiveView={setActiveView}
          isOpen={isSidebarOpen}
        ...
        <main className="flex-1 p-4 sm:p-6 lg:p-8 overflow-y-auto">
          ...
          <button
            onClick={() => setIsSidebarOpen(true)}
            className="md:hidden p-2 mb-4 text-gray-600 hover:text-gray-900"
            aria-label="Open sidebar"
          >
            <Bars3Icon className="h-6 w-6" />
          </button>
      ...
      {/* Footer can be removed if the h-screen layout is preferred without it */}
      <Footer /> 
    </div>
  );
}