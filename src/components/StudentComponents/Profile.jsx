...
            panNumber: profileData.panNumber || '', contactNumber: profileData.contactNumber || '',
            // --- CHANGE: Check for the new Cloudinary URL fields ---
            class10Certificate: profileData.class10CertificateUrl ? { name: 'Uploaded Certificate' } : null,
            class12Certificate: profileData.class12CertificateUrl ? { name: 'Uploaded Certificate' } : null,
            graduationCertificate: profileData.graduationCertificateUrl ? { name: 'Uploaded Certificate' } : null,
          });
          setShowProfile(true);
        }
      } catch (error) {
...
        alert('Profile saved successfully!');
        // --- CHANGE: Update state after save to reflect new Cloudinary URLs ---
        const newProfileData = res.data.profile;
        setFormData({
            fullName: newProfileData.fullName || '', fatherName: newProfileData.fatherName || '',
            motherName: newProfileData.motherName || '', universityName: newProfileData.universityName || '',
            graduationYear: newProfileData.graduationYear || '', aadharNumber: newProfileData.aadharNumber || '',
            panNumber: newProfileData.panNumber || '', contactNumber: newProfileData.contactNumber || '',
            class10Certificate: newProfileData.class10CertificateUrl ? { name: 'Uploaded Certificate' } : null,
            class12Certificate: newProfileData.class12CertificateUrl ? { name: 'Uploaded Certificate' } : null,
            graduationCertificate: newProfileData.graduationCertificateUrl ? { name: 'Uploaded Certificate' } : null,
        });
        setShowProfile(true);
      }
    } catch (error) {
...
    const requiredFieldPoints = 50;
    const requiredDocPoints = 30;
    const optionalPoints = 20;

    requiredFields.forEach(field => { if (formData[field]?.toString().trim()) { score += requiredFieldPoints / requiredFields.length; } });
    if (formData.class10Certificate) score += requiredDocPoints / 2;
    if (formData.class12Certificate) score += requiredDocPoints / 2;
    if (formData.universityName) score += optionalPoints / 3;
    if (formData.graduationYear) score += optionalPoints / 3;
    if (formData.graduationCertificate) score += optionalPoints / 3;
...
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Full Name <span className="text-red-500">*</span></label><input type="text" placeholder="Enter your full name" value={formData.fullName} onChange={(e) => handleInputChange('fullName', e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900 placeholder:text-gray-500" /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Father's Name <span className="text-red-500">*</span></label><input type="text" placeholder="Enter father's name" value={formData.fatherName} onChange={(e) => handleInputChange('fatherName', e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900 placeholder:text-gray-500" /></div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Mother's Name <span className="text-red-500">*</span></label><input type="text" placeholder="Enter mother's name" value={formData.motherName} onChange={(e) => handleInputChange('motherName', e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900 placeholder:text-gray-500" /></div>
                </div>
              </section>
              <section>
...
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Graduation Year</label><select value={formData.graduationYear} onChange={(e) => handleInputChange('graduationYear', e.target.value)} className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900"><option value="">Select graduation year</option>{graduationYears.map((year) => (<option key={year} value={year}>{year}</option>))}</select></div>
                </div>
              </section>
              <section>
...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Aadhar Number <span className="text-red-500">*</span></label><input type="text" placeholder="Enter 12-digit Aadhar number" value={formData.aadharNumber} onChange={(e) => handleInputChange('aadharNumber', e.target.value.replace(/\D/g, '').slice(0, 12))} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900 placeholder:text-gray-500 ${errors.aadharNumber ? 'border-red-500' : 'border-gray-300'}`} />{errors.aadharNumber && <p className="text-red-500 text-sm mt-1">{errors.aadharNumber}</p>}</div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">PAN Number <span className="text-red-500">*</span></label><input type="text" placeholder="Enter PAN number" value={formData.panNumber} onChange={(e) => handleInputChange('panNumber', e.target.value.toUpperCase().slice(0, 10))} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 font-mono text-gray-900 placeholder:text-gray-500 ${errors.panNumber ? 'border-red-500' : 'border-gray-300'}`} />{errors.panNumber && <p className="text-red-500 text-sm mt-1">{errors.panNumber}</p>}</div>
                  <div><label className="block text-sm font-semibold text-gray-700 mb-2">Contact Number <span className="text-red-500">*</span></label><input type="tel" placeholder="Enter 10-digit contact number" value={formData.contactNumber} onChange={(e) => handleInputChange('contactNumber', e.target.value.replace(/\D/g, '').slice(0, 10))} className={`w-full px-4 py-3 border rounded-lg focus:ring-2 focus:ring-blue-500 text-gray-900 placeholder:text-gray-500 ${errors.contactNumber ? 'border-red-500' : 'border-gray-300'}`} />{errors.contactNumber && <p className="text-red-500 text-sm mt-1">{errors.contactNumber}</p>}</div>
                </div>
              </section>
              <section>
