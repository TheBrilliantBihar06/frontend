...
export default function App() {
  ...
  return (
    <div className="min-h-screen  font-sans bg-gradient-to-br from-gray-50 to-black-100">
      <Navbar />
...
const Form = () => {
  ...
  useEffect(() => {
    if (formData.category) {
      if (formData.category === 'sc_st') {
        // SC/ST: ₹200 for all genders
        setPaymentRequired(true);
        setPaymentAmount(2);
      } else if (['obc', 'general'].includes(formData.category)) {
        // For OBC/General: Check gender
  ...
  const handleSendOtp = async () => {
    ...
    setOtpMessage({ text: '', type: '' });
    try {
      await axios.post('https://backend-production-fe57.up.railway.app/api/otp/send', { email: formData.email });
      setIsOtpSent(true);
  ...
  const handleVerifyOtp = async () => {
    ...
    setOtpMessage({ text: '', type: '' });
    try {
      await axios.post('https://backend-production-fe57.up.railway.app/api/otp/verify', { email: formData.email, otp });
  ...
  const handlePayment = async () => {
    ...
    setLoading(true);
    try {
      const { data } = await axios.post('https://backend-production-fe57.up.railway.app/api/payment/create-order', {
  ...
  const submitForm = async () => {
    ...
    try {
      ...
      const response = await axios.post("https://backend-production-fe57.up.railway.app/api/application/submit", formDataToSend);
      setLoading(false);
